...
int main(int argc, char** argv) {
    ...
    Vector3 wallC(0, 1.5, 0), wallHalf(2.2, 1.5, 0.12);
    ...
    for (auto& d : descs) {
        ...
        f.body = std::make_unique<RigidBody>(); f.body->setMass(vol * 2.4);        // concrete density
        ...
        float j = (cr() - 0.5f) * 0.10f;
        f.colour = glm::vec3(0.58f + j, 0.57f + j, 0.55f + j);
        frags.push_back(std::move(f));
    }

    // ---- the metal ball
    RigidBody ball; ball.setMass(140); real rr = 0.42;
    Matrix3 Ib; real cI = 0.4 * 140 * rr * rr; Ib.setInertiaTensorCoeffs(cI, cI, cI); ball.setInertiaTensor(Ib);
    ball.setAcceleration(0, -1.2, 0); ball.setDamping(0.999, 0.995);
    ball.setPosition(0, 1.62, -7.0); ball.setVelocity(0, 0.35, 16.0);             // flat shot at the wall centre
    ...
    gfx::App app(W, H, "wall breakthrough", headless);
    gfx::Renderer r; r.init(W, H);
    gfx::Mesh box = gfx::makeBox(), sphere = gfx::makeSphere(), plane = gfx::makePlane(40, 1);
    gfx::OrbitCamera cam; cam.target = glm::vec3(0, 1.3, 0); cam.dist = 9.5f; cam.yaw = 2.75f; cam.pitch = 0.16f;
    ...
    auto stepPhysics = [&]() {
        ...
        if (!punched && ball.getPosition().z > -wallHalf.z - rr - 0.02) {
            ...
            real R = 0.72; real ph1 = hr() * 6.28, ph2 = hr() * 6.28;
            Destructible dest;
            for (auto& f : frags) {
            ...
            dest.shatter(impact - Vector3(0, 0, 0.4), Vector3(0, 0.5, 3.4), 1.6, 8, 21u);
    ...
    auto renderFrame = [&]() {
        ...
        for (auto& f : frags) { glm::mat4 m = glm::scale(glmFromPhys(f.body->getTransform()), glm::vec3(f.half.x, f.half.y, f.half.z));
            r.drawPBR(r.pSolid, box, m, f.colour, 0.85f, 0.0f); }
