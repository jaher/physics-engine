...
| + | — | **Destruction & explosions** — grid/jitter box fracture, **Voronoi** irregular convex chunks, radial+concentric **impact ("bullet-hole") glass fracture**, or **hollow-cylinder shell fracture** into small curved metal shards (all with exact volume/COM/inertia); `detonate()` a charge for a radial blast-overpressure explosion with an upward plume | `fracture.h`, `voronoi.h`, `glassfrac.h`, `shellfrac.h` |
...
(`tests/*.cpp`, **573 assertions, all passing** — 23 suites, incl. a CUDA GPU suite):
...
| `bulletbarrel3d` | a **slow-motion chain reaction** — a bullet drifts through a glass window in bullet-time trailing a glowing **corkscrew vapor spiral**, shatters the pane (impact fracture), carries on and strikes a **hollow explosive barrel**; on the hit time ramps back to normal, the barrel bursts into **small curved metal shards** (shell fracture) and the blast **shatters the crates, pillars and blocks** around it. Four fracture types in one shot (glass impact + barrel shell + object Voronoi), all convex/GJK-EPA, contained by the room walls, with an HDR fireball + dust ([`docs/bulletbarrel.mp4`](docs/bulletbarrel.mp4)) |
