...
| + | — | **Destruction & explosions** — grid/jitter fracture into box fragments, **Voronoi fracture into irregular convex-polyhedron chunks**, or a radial+concentric **impact ("bullet-hole") fracture** of a thin pane (exact volume/COM/inertia); `detonate()` a charge for a radial blast-overpressure explosion with an upward plume | `fracture.h`, `voronoi.h`, `glassfrac.h` |
...
(`tests/*.cpp`, **567 assertions, all passing** — 23 suites, incl. a CUDA GPU suite):
...
| `explosion` | grid fracture conserves the block's volume and fragment count exactly; **Voronoi fracture partitions the block (Σ cell volume ≈ box) into convex cells with valid mass properties**; the radial+concentric **glass fracture** likewise tiles the pane (minus the punched hole) into thin convex shards; `detonate()` throws every fragment radially outward with a net upward plume momentum, its blast energy scales with the square of the charge strength, and fragments outside the blast radius stay asleep |
